...
import {
  OpenCodeDriver,
  buildOpenCodeArgs,
} from "../dist/drivers/OpenCodeDriver.js";
...
describe("OpenCodeDriver Integration", () => {
  ...
  const fakeOpenCodePath = path.join(
    fakeBinDir,
  ...
  function writeFakeOpenCode() {
    ...
    if (process.platform === "win32") {
      ...
      fs.writeFileSync(
        ...
        [
          "#!/bin/sh",
          'printf "%s\\n" "FAKE OPENCODE $*" "<<<FILE:hello.js>>>" "console.log(\"Hello, world!\");" "<<<END>>>"',
        ].join("\n"),
        "utf8",
      );
  ...
  test("resolveOpenCodeCommand prefers OPENCODE_PATH and then vibe_config.json", () => {
    ...
    const configOnlyPath = path.join(
      configDir,
      process.platform === "win32" ? "config-opencode.cmd" : "config-opencode",
    );

    fs.mkdirSync(configDir, { recursive: true });
    fs.writeFileSync(
      path.join(configDir, "vibe_config.json"),
      JSON.stringify({
        opencodePath:
          "./config-opencode" + (process.platform === "win32" ? ".cmd" : ""),
      }),
      "utf8",
    );
    fs.writeFileSync(
    ...
    const envResolved = resolveOpenCodeCommand({
      cwd: configDir,
      env: { OPENCODE_PATH: path.resolve(fakeOpenCodePath) },
    });
  ...
  test("ExecuteTask uses custom executable path, task.workDir, and file block output", async () => {
    ...
    assert.ok(
      executableLog?.includes(path.resolve(fakeOpenCodePath)),
      "Should log the resolved executable path",
    );
    assert.ok(
      runLog?.includes("run --agent build"),
      "Command should include build agent for implementation work",
    );
    assert.ok(
      fs.existsSync(path.join(workDir, "hello.js")),
      "File block output should be written into task.workDir",
    );
    assert.equal(
      fs.existsSync(path.join(testDir, "task-101", "hello.js")),
      false,
      "Fallback task directory should not be used when workDir is present",
    );
  });
  ...
  test("ExecuteTask with custom model passes --model flag", async () => {
    ...
    assert.ok(
      runLog.includes("--model ollama:llama3"),
