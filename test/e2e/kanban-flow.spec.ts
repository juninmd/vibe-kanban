...
test.describe("Kanban fluxo principal", () => {
  test("cria card, alterna para visão 2D e exibe no board", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    await page
      .getByRole("button", { name: "Adicionar card", exact: true })
      .click();
    await page.getByRole("button", { name: "Alternar 2D / 3D" }).click();

    await expect(page.locator("#view2d")).toHaveClass(/active/);
    await expect(page.locator("#kanbanBoard")).toContainText(
      "E2E - hardening produção",
    );
  });
