...
export const DB = {
  ...
  addTerminalLog(agentId: string, taskId: number | null, type: string, content: string): void {
    db.prepare('INSERT INTO terminal_logs (agentId, taskId, type, content, timestamp) VALUES (?, ?, ?, ?, ?)').run(
      agentId,
      taskId,
      type,
      content,
  ...
  getTerminalLogs(
    ...
    return db
      .prepare('SELECT type, content, timestamp, taskId FROM terminal_logs WHERE agentId = ? ORDER BY id DESC LIMIT ?')
  ...
  getTaskTerminalLogs(
    ...
    return db
      .prepare('SELECT type, content, timestamp, agentId FROM terminal_logs WHERE taskId = ? ORDER BY id ASC LIMIT ?')
