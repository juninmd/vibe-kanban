...
export async function createPullRequest(
  ...
  const response = await fetch(
    `https://api.github.com/repos/${normalizedRepo}/pulls`,
    {
      method: 'POST',
      headers: {
  ...
  if (!response.ok) {
    ...
    if (
      ...
      errorData.errors.some((e: any) =>
        e && typeof e.message === 'string' && e.message.indexOf('A pull request already exists') !== -1
      )
    ) {
      return `Pull Request já existe para a branch ${branchName}.`;
    }
    throw new Error(
      `Falha ao criar PR na API do GitHub: ${response.status} ${response.statusText} - ${JSON.stringify(errorData)}`,
    );
  }
