...
export function buildGithubRemoteUrl(
  githubRepo: string,
  githubToken?: string,
): string {
  const normalizedRepo = normalizeGithubRepo(githubRepo);
...
export async function prepareWorktree(
  ...
  if (!fs.existsSync(baseRepoDir)) {
    ...
    await runGit(
      [
        ...
        'add',
        'origin',
        buildGithubRemoteUrl(normalizedRepo, githubToken),
      ],
      baseRepoDir,
    );
  }
  ...
  try {
    ...
    console.warn(
      `[Worktree] Failed to fetch origin for ${normalizedRepo}, it might be an empty repository or require authentication.`,
    );
  }
  ...
  try {
    ...
    try {
      // Try origin/master
      await runGit(
        ['worktree', 'add', '-b', branchName, worktreeDir, 'origin/master'],
        baseRepoDir,
      );
    } catch (e2) {
