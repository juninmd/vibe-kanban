...
function buildBusinessRequirements(title: string, description?: string): string[] {
  ...
  if (normalized.includes("seguran") || normalized.includes("security")) {
    baseRequirements.push("Exigir evidência de scans SAST/Dependency em toda entrega de segurança.");
  }

  if (normalized.includes("saas") || normalized.includes("multi-tenant")) {
    baseRequirements.push("Garantir isolamento multi-tenant para dados, contexto e segredos de agentes.");
  }

  if (normalized.includes("checkout") || normalized.includes("pagamento")) {
    baseRequirements.push("Incluir observabilidade de funil de receita com alertas de conversão em tempo real.");
  }
...
export function enrichDemand(input: DemandIntakeInput) {
  ...
  const businessRequirements = buildBusinessRequirements(input.title, input.description);
