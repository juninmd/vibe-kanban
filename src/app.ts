  ...
  lanes.forEach((lane) => {
    ...
    tasks
      ...
      .forEach((task) => {
        ...
        if (lane === "backlog") actions.append(makeBtn("Pegar", () => pickTask(task)));
        if (lane === "in_progress") actions.append(makeBtn("Parar", () => interruptTask(task)));
...
// Server Racks (Decor)
function createServerRack(x: number) {
...
const racks: any[] = [createServerRack(-10), createServerRack(-8), createServerRack(8), createServerRack(10)];
