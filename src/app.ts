  ...
  try {
    ...
    if (data.tools && data.tools.length > 0) {
      els.agentTool.innerHTML =
        ...
        data.tools
          .map((t: any) => `<option value="${t.id}"${t.id === agent.tool ? ' selected' : ''}>${t.name}</option>`)
          .join('');
    }
  } catch (e) {
    console.error(e);
    ...
    try {
      ...
      if (data.models && data.models.length > 0) {
        els.agentModelDropdown.innerHTML = data.models
          .map((m: string) => `<option value="${m}"${m === agent.model ? ' selected' : ''}>${m}</option>`)
          .join('');
      }
    } catch (e) {
      console.error(e);
...
function spawnConfetti() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const color = new THREE.Color().setHSL(secureRandom(), 0.9, 0.6);
    colors[i * 3] = color.r;
    colors[i * 3 + 1] = color.g;
    colors[i * 3 + 2] = color.b;

    velocities.push({
      x: (secureRandom() - 0.5) * 0.3,
      y: secureRandom() * 0.3 + 0.1,
      z: (secureRandom() - 0.5) * 0.3,
    });
  }
  ...
  agentMeshes.forEach((item) => {
    ...
    if (item.phase === 'working' && agentData?.assignedTask && item.laser) {
      ...
      if (cardMesh && taskObj) {
        ...
        const lastLog =
          taskObj.logs && taskObj.logs.length > 0 ? taskObj.logs[taskObj.logs.length - 1] : 'Buscando contexto...';
...
function spawnTrail(position: THREE.Vector3, color: THREE.Color) {
  ...
  mesh.position.x += (secureRandom() - 0.5) * 0.2;
  mesh.position.z += (secureRandom() - 0.5) * 0.2;
  scene.add(mesh);
