...
Do not output hypothetical logs. Output the actual file content needed to solve the task.`;
    ...
    if (agent.tool === 'opencode' && this.terminalManager) {
      ...
      try {
        ...
        ctx.onLog(
          task.id,
          `Command sent to PTY. Monitor the terminal panel for output.`,
        );
    ...
    try {
      ...
      timeoutHandle = setTimeout(() => {
        ...
        if (!child.killed) {
          ...
          this.appendLog(
            task.id,
            `[TIMEOUT] Process killed after exceeding ${SESSION_TIMEOUT_MS / 1000}s`,
          );
          ctx.onLog(
            task.id,
            `[TIMEOUT] Process killed after exceeding ${SESSION_TIMEOUT_MS / 1000}s`,
      ...
      child.on('close', (code) => {
        ...
        if (isTimedOut) {
          ...
          ctx.onLog(
            task.id,
            `Process completed. Files created: ${filesCreated}`,
          );
          ctx.onComplete(task.id);
        } else {
