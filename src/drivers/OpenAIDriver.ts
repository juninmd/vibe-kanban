...
export class OpenAIDriver implements LLMDriver {
  ...
  async executeTask(
    ...
    if (!process.env.OPENAI_API_KEY) {
      throw new Error(
        'OPENAI_API_KEY not found in environment. Please configure it in Settings.',
      );
    }
...
Do not output hypothetical logs. Output the actual file content needed to solve the task.`;
    ...
    ctx.onLog(
      task.id,
      `Starting OpenAI Agent with model ${agent.model || 'gpt-4o'}...`,
    );
    logDebugBlock(
      ctx,
      task.id,
    ...
    try {
      ...
      if (filesCreated === 0) {
        ctx.onLog(
          task.id,
          'No files were created. Response: ' +
