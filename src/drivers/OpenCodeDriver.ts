...
const MISSING_OPENCODE_MESSAGE =
  "OpenCode CLI not found. Set OPENCODE_PATH, add opencodePath to vibe_config.json, or install opencode globally in PATH.";
...
`;
    ...
    logDebugCommand(ctx, task.id, openCode.command, [
      ...args.slice(0, -1),
      "<prompt>",
    ]);
    ctx.onLog(
      task.id,
      `[SYSTEM] OpenCode executable: ${openCode.command} (${openCode.source})`,
    );
    ...
    const errorLoopDetector = createErrorLoopDetector(
      proc,
      OPENCODE_ERROR_PATTERN,
    );
    const stallDetector = createStallDetector(proc, 120);
    const overseer = startOverseer(proc, taskDir, {
      enabled: true,
      check_interval: 30,
      stuck_threshold: 300,
    });
